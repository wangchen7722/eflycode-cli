...
class ListFilesTool(BaseTool):
    ...
    def description(self) -> str:
        return """ 
     查看指定目录中的文件和目录。支持递归遍历子目录，可以根据.echoignore文件中定义的规则过滤不需要的文件和目录。
     显示每个文件的详细信息包括文件大小，以及每个目录包含的文件和子目录数量统计。
     """
...
class ReadFileTool(BaseTool):
    ...
    def description(self) -> str:
        return """
        读取指定路径文件的内容。用于查看现有文件的内容，例如分析代码、查看文本文件或从配置文件中提取信息。
        输出包含行号前缀（例如 "1 | const x = 1"）。默认每次显示100行，如果文件超过100行会显示剩余行数。
        """
...
class EditFileTool(BaseTool):
    ...
    @property
    def description(self) -> str:
        return """在指定路径的文件中进行搜索替换或插入操作。支持两种模式：
